...
class BINGtalkStyledApp(tk.Tk):
    ...
    def load_icons(self):
        ...
        for name in icon_names:
            path = os.path.join("/Users/justinbian/PycharmProjects/hand_written_digit_recognition_CNN/chat_system_gui/icons", f"{name}.png")
            try:
    ...
    def launch_server():
        """Spawn the Flask server in a background thread."""
        def run():
            # Change directory to wherever app.py lives
            os.chdir('/path/to/your/project')
            # Use sys.executable so it picks the same Python interpreter
    ...
    # --- Simple Tkinter GUI hookup ---
    root = tk.Tk()
    root.title("My Chat + Digit Recognizer Launcher")

    launch_btn = tk.Button(root, text="Launch Digit Recognizer Demo", command=launch_server)
    launch_btn.pack(padx=20, pady=20)
