...
from the_chat_system.client.client_state_machine import ClientSM
...
class BINGtalkStyledApp(tk.Tk):
    ...
    def initialize_network(self):
        try:
            ...
            self.client_sm = ClientSM(self.s)
            # Login
            self.username = simpledialog.askstring("Login", "Enter your username:", parent=self)
            ...
            if response.get("status") == "ok":
                ...
                messagebox.showerror("Login Failed", response.get("msg", "Duplicate username"))
                self.destroy()
    ...
    def show_tools(self):
        ...
        # Tools UI elements...
        # Time button
        tk.Button(self.content_frame, text="Time", width=25, height=2, bg="#f0f0f0",
                  command=lambda: self.send_command('time')).pack(pady=5)

        # Get Poem button
        tk.Button(self.content_frame, text="Get Poem", width=25, height=2, bg="#f0f0f0",
                  command=self.handle_poem).pack(pady=5)
